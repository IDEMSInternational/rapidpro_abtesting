...
class FlowSnippet(object):
    ...
    def __init__(
        self, nodes, nodes_layout, node_variations, original_node, root_uuid=-1
    ):
        self._nodes = nodes
...
class GenericEditOp(ABC):
    ...
    def create_edit_op(
        ...
        return class_name(
            flow_id,
            row_id,
            parsed_node_identifier,
            bit_of_text,
            split_by,
            default_text,
            debug_string,
            has_node_for_other_category,
            assign_to_group,
            uuid_lookup,
            config,
        )
        ...
        for action in node["actions"]:
            if action["type"] == "send_msg":
                if self._node_match_regex:
                    result |= bool(
                        re.fullmatch(
                            self.node_identifier(), action["text"], flags=re.DOTALL
                        )
                    )
                else:
                    result |= self._lenient_text_match(
                        action["text"], self.node_identifier()
                    )
        return result
    ...
    def _construct_match_cases(self, node):
        ...
        for node_case in router["cases"]:
            category = next(
                ...
                if cat["uuid"] == node_case["category_uuid"]
            )
            case = {
                "type": node_case["type"],
    ...
    def _matches_wait_for_response_cases(self, node):
        ...
        if (
            "wait" not in router
            or router["type"] != "switch"
            or not router["operand"].startswith("@input")
        ):
            return False
    ...
    def _matching_save_value_action_id(self, node):
        ...
        for i, action in enumerate(node["actions"]):
            if (
                ...
                and "@results." + action["name"].lower().replace(" ", "_")
                == self.node_identifier()
                and str(self.bit_of_text()) == str(action["value"])
            ):
                return i
            if (
                action["type"] == "set_contact_field"
                and "@fields." + action["field"]["key"].lower()
                == self.node_identifier()
                and str(self.bit_of_text()) == str(action["value"])
            ):
                return i
            if (
                action["type"] == "set_contact_name"
                and "@contact.name" == self.node_identifier()
                and str(self.bit_of_text()) == str(action["name"])
            ):
                return i
            if (
                action["type"] == "set_contact_channel"
                and "@contact.channel" == self.node_identifier()
                and str(self.bit_of_text()) == str(action["channel"]["name"])
            ):
                return i
            if (
                action["type"] == "set_contact_language"
                and "@contact.language" == self.node_identifier()
                and str(self.bit_of_text()) == str(action["language"])
            ):
                return i
            if (
                action["type"] == "set_contact_status"
                and "@contact.status" == self.node_identifier()
                and str(self.bit_of_text()) == str(action["status"])
            ):
                return i
        return -1
...
class FlowEditOp(GenericEditOp):
    ...
    def _replace_text_in_message(self, node, replacement_text):
        ...
        if total_occurrences == 0:
            ...
            logger.warning(
                self.debug_string()
                + 'No occurrences of "{}" found node.'.format(self.bit_of_text())
            )
        if total_occurrences >= 2:
            logger.warning(
                self.debug_string()
                + 'Multiple occurrences of "{}" found in node.'.format(
                    self.bit_of_text()
                )
            )
    ...
    def _replace_entered_flow(self, node, replacement_text):
        if self._matches_entered_flow(node):
            ...
            logger.warning(
                self.debug_string()
                + 'No occurrences of "{}" found in node.'.format(self.bit_of_text())
            )
    ...
    def _replace_in_action_list_field(self, node, replacement_text, action_field):
        ...
        for bit_of_text, repl_text in zip(
            self._bit_of_text.split(";"), replacement_text.split(";")
        ):
            total_occurrences = 0
            for action in node["actions"]:
                if action["type"] == "send_msg":
    ...
    def _replace_wait_for_response_cases(self, node, replacement_text):
        ...
        try:
            ...
            logger.warning(
                self.debug_string()
                + 'Malformed replacement value "{}" (should be JSON).'.format(
                    replacement_text
                )
            )
            return
        if type(replacement_cases) != list or len(replacement_cases) != len(
            ...
            logger.warning(
                self.debug_string()
                + 'Replacement should be list of length {} but is "{}".'.format(
                    len(node["router"]["cases"]), replacement_text
                )
            )
            return
        for case, node_case in zip(replacement_cases, node["router"]["cases"]):
            if not {"category_name", "type", "arguments"}.issubset(case.keys()):
                logger.warning(
                    self.debug_string()
                    + 'Skipping invalid replacement case "{}".'.format(case)
                )
                continue
            category = next(
                cat
                for cat in node["router"]["categories"]
                if cat["uuid"] == node_case["category_uuid"]
            )
    ...
    def _get_assigntogroup_gadget(self, node):
        if len(self.categories()) != 2:
            logger.warning(
                self.debug_string()
                + "assign_to_group only for A/B tests (i.e. 2 groups)."
            )
            return None, None
        ...
        if group_assignment == "always A":
            gadget, gadget_layout = get_assign_to_fixed_group_gadget(
                groupA_name, groupA_uuid, node["uuid"]
            )
        elif group_assignment == "always B":
            gadget, gadget_layout = get_assign_to_fixed_group_gadget(
                groupB_name, groupB_uuid, node["uuid"]
            )
        else:  # group_assignment == "random":
            gadget, gadget_layout = get_assign_to_group_gadget(
                groupA_name, groupA_uuid, groupB_name, groupB_uuid, node["uuid"]
            )
    ...
    def _get_variation_tree_snippet(self, input_node, node_layout):
        ...
        if len(node_variations) == 1:
            ...
            full_layout = NodesLayout.from_single_node_layout(
                input_node["uuid"], node_layout
            )
        else:
            ...
            full_layout = make_tree_layout(
                self.split_by, switch_node["uuid"], node_variations, node_layout
            )
        ...
        return FlowSnippet(
            all_nodes, full_layout, node_variations, input_node, first_node["uuid"]
        )
...
class ReplaceFlowFlowEditOp(FlowEditOp):
    ...
    def _process_uuid_lookup(self, uuid_lookup):
        ...
        if flow_uuid is None:
            logger.warning(
                self._debug_string + "Flow " + flow_name + " does not exist."
            )
        self._default_text = {
            "uuid": flow_uuid,
            "name": flow_name,
...
class TranslationEditOp(GenericEditOp):
    ...
    def _replace_text_in_message(self, localization, node):
        ...
        for action in node["actions"]:
            if action["type"] == "send_msg":
                ...
                if "text" not in tr_action:
                    logger.warning(
                        self.debug_string()
                        + f'Translation of action "{action["uuid"]}" has no text.'
                    )
                    continue
                text = tr_action["text"][
                    0
        ...
        if total_occurrences == 0:
            ...
            logger.warning(
                self.debug_string()
                + 'No occurrences of "{}" found node translation.'.format(
                    self.bit_of_text()
                )
            )
        if total_occurrences >= 2:
            logger.warning(
                self.debug_string()
                + 'Multiple occurrences of "{}" found in node translation.'.format(
                    self.bit_of_text()
                )
            )
    ...
    def _replace_in_action_list_field(self, localization, node, action_field):
        for bit_of_text, repl_text in zip(
            self.bit_of_text().split(";"), self.default_text().split(";")
        ):
            total_occurrences = 0
            for action in node["actions"]:
                if action["type"] == "send_msg":
    ...
    def _replace_wait_for_response_cases(self, localization, node):
        ...
        for case, node_case in zip(replacement_cases, node["router"]["cases"]):
            if not {"arguments"}.issubset(case.keys()):
                logger.warning(
                    self.debug_string()
                    + 'No case arguments provided for tranlsation "{}".'.format(case)
                )
            else:
                ...
                if not tr_case:
                    logger.warning(
                        self.debug_string()
                        + f'Translation of case "{node_case["uuid"]}" does not exist.'
                    )
                elif "arguments" not in tr_case:
                    logger.warning(
                        self.debug_string()
                        + f'Translation of case "{node_case["uuid"]}" has no arguments.'
                    )
                else:
            ...
            if not {"category_name"}.issubset(case.keys()):
                logger.warning(
                    self.debug_string()
                    + 'No category name for tranlsation "{}".'.format(case)
                )
                continue
            node_category = next(
                cat
            ...
            if "name" not in tr_category:
                logger.warning(
                    self.debug_string()
                    + f'Translation of category "{node_category["uuid"]}" has no name.'
                )
                continue
