...
def get_send_msg_node(uuid, text):
    node = {
        ...
        "exits": [
            {
                "uuid": generate_random_uuid(),
                "destination_uuid": generate_random_uuid()
            }
        ]
    }
...
def make_nodes(languages, data):
    ...
    for lid, lang in enumerate(languages):
        ...
        # Error message node
        node, error_next_node_uuid = get_send_msg_node(error_next_node_uuid, data[rowid][2])
        ...
        switcher_node["exits"].append(
            {
                "uuid": exit_uuid,
                "destination_uuid": set_contact_nodes[lid]["uuid"]
            })
    ...
    switcher_node["exits"].append(
        {
            "uuid": exit_uuid,
            "destination_uuid": error_nodes[0]["uuid"]
        })
